...
def get_class_id(root_path, train_names):
    ...
    for id, person_name in enumerate(train_names):
        ...
        for person_img_sample in os.listdir(person_dir):
            ...
            ratio = 800 / w
            new_h = h * ratio
            img = cv2.resize(img, (800, int(new_h)))
...
def draw_prediction_results(verification_statuses, test_image_list, test_faces_rects, train_names):
    ...
    for ver, img, (x, y, w, h) in zip(verification_statuses, test_image_list, test_faces_rects):
        ...
        if ver[1] == 1:
            ...
            img = cv2.putText(img, ver[0], (x, y - 10), font, 1.8, (0, 255, 0), 3, cv2.LINE_AA)
            img = cv2.putText(img, "Verified", (x, y + h + 55), font, 2, (0, 255, 0), 3, cv2.LINE_AA)

        else:
            img = cv2.rectangle(img, (x, y), (x + w, y + h), (0, 0, 255), 3)
            img = cv2.putText(img, ver[0], (x, y - 10), font, 1.8, (0, 0, 255), 3, cv2.LINE_AA)
            img = cv2.putText(img, "Unverified", (x, y + h + 55), font, 2, (0, 0, 255), 3, cv2.LINE_AA)
...
if __name__ == "__main__":
    ...
    predicted_test_image_list = draw_prediction_results(verification_statuses, test_image_list, test_faces_rects,
                                                        train_names)
